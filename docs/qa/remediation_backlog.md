...
| 3 | QA-103 add transformer CI — local replay passed, remote review pending | QA-CI-001 | independent hash-locked Linux standard/CPU-transformer jobs; offline synthetic CPU evidence; GPU acceptance explicitly excluded | 293-test standard and 294-test transformer local replays pass; both required remote jobs must pass | blocking remotely |
| 4 | QA-104 protect evidence workflow | QA-GIT-001 | protected `main` with required CI and no force-push/deletion | GitHub branch API reports protected | recommended before paper commit |
